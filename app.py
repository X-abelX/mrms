...
def add_palet():
    if request.method == 'POST':
        ...
        conn = mysql.connection.cursor()
        conn.execute('INSERT INTO palets (albaran, contacto, direccion, fecha_entrega, numpalets, peso, tipo_palet, nota, despaletizado) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)', (id,contacto,direccion,fecha_entrega,num_palets,peso,tipo_palet,nota,despaletizado))
...
def assign_palets():
    if request.method == 'POST':
        ...
        for palet_id in selected_palets:
            conn.execute('UPDATE palets SET empresa = %s WHERE id = %s', ("mam",palet_id))
        mysql.connection.commit()
        return redirect(url_for('home'))
...
def login():
    if request.method == 'POST':
        ...
        conn = mysql.connection.cursor()
        conn.execute('SELECT * FROM `users` WHERE `nombre` = %s AND `password` = %s', (username, password))
...
def add_employed():
    if request.method == 'POST':
        ...
        conn = mysql.connection.cursor()
        conn.execute('INSERT INTO users (rol, nombre, correo, contacto) VALUES (%s, %s, %s, %s)', ("empleado",nombre,correo,contacto))
        mysql.connection.commit()
        return redirect(url_for('admin'))
...
def assign_empolyed():
    if request.method == 'POST':
        ...
        conn = mysql.connection.cursor()
        for palet_id in selected_palets:
            conn.execute('UPDATE palets SET empleado = %s WHERE id = %s', (str(employed),palet_id))
        mysql.connection.commit()
